...
Seven public functions form two pipelines:

  ── Structured data (NYC TLC) ──────────────────────────────────────────────
  ...
  ── Unstructured text (Airbnb NLP) ─────────────────────────────────────────
  run_embedding_drift_report          →  Evidently Report on PCA-reduced BERT embeddings
  parse_embedding_drift_results       →  plain dict of embedding drift statistics
...
Embedding drift detects shifts in the contextual representation space of a fine-tuned
BERT model, which can signal that the language distribution has changed even when
structured features appear stable.
...
run_embedding_drift_report accepts raw numpy arrays of shape (N, embedding_dim).
"""
...
# ── Embedding drift (NLP pipeline) ────────────────────────────────────────────

def run_embedding_drift_report(
    ref_embeddings: np.ndarray,
    cur_embeddings: np.ndarray,
    n_components:   int = 20,
) -> Report:
    """
    Detect distribution shift in BERT [CLS] embedding space.

    Raw BERT embeddings are 768-dimensional — running Evidently's drift tests
    on all 768 columns is slow and produces an unreadable report.  We first
    reduce to n_components principal components (capturing ~80-90 % of variance
    for typical review corpora), then apply DataDriftPreset on the compact
    representation.

    Args:
        ref_embeddings : (N_ref, 768) array from extract_embeddings() on reference data
        cur_embeddings : (N_cur, 768) array from extract_embeddings() on current data
        n_components   : PCA dimensions to retain before running Evidently

    Returns:
        Evidently Report object.  Call .save_html(path) for the interactive dashboard.
    """
    ...
    ref_df = pd.DataFrame(ref_reduced, columns=cols)
    cur_df = pd.DataFrame(cur_reduced, columns=cols)
...
def parse_embedding_drift_results(report: Report) -> dict:
    """
    Extract embedding drift statistics from a run_embedding_drift_report() output.

    Returns a dict with:
      overall_drift  (bool)  — True if dataset-level drift detected in embedding space
      share_drifted  (float) — fraction of PCA components that drifted (0–1)
      n_drifted      (int)   — number of drifted PCA components
    """
    metrics        = report.as_dict()["metrics"]
    dataset_metric = next(m for m in metrics if m["metric"] == "DatasetDriftMetric")
    overall        = dataset_metric["result"]
